...
@section('content')
    ...
    <header class="flex flex-col items-center gap-[50px] sm:max-2xl:mt-[70px] mt-[30px]">
        <div id="Headline" class="max-w-[1130px] mx-[20px] sm:max-lg:mx-[20px] lg:mx-auto flex flex-col gap-4 items-center">
            <p class="w-fit text-[#A3A6AE] sm:max-2xl:text-sm text-[12px]">{{ $post->created_at->format('d M, Y') }} • {{ $post->category->name }}</p>
            <h1 id="Title" class="sm:max-2xl:text-4xl text-2xl leading-relaxed font-bold text-center">{{ $post->title }}
            </h1>
                ...
                <a id="Author" href="{{ url('/author/' . $post->author->name) }}" class="w-fit h-fit">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full overflow-hidden">
                            <img src="{{ asset('storage/' . $post->author->photo) }}" class="object-cover w-full h-full"
                                alt="avatar">
                        </div>
                        <div class="flex flex-col">
                            <p class="font-semibold text-sm leading-[21px]">{{ $post->author->name }}</p>
                            <p class="text-xs leading-[18px] text-[#A3A6AE]">
                                {{ $authorRole->author?->getRoleNames()?->first() ?? 'No Role' }}
                            </p>
                        </div>
                    </div>
                </a>
        ...
        <div class="w-full sm:max-2xl:h-[500px] h-[250px] flex shrink-0 overflow-hidden">
            <img src="{{ asset('storage/' . $post->thumbnail) }}" class="object-cover w-full h-full" alt="cover thumbnail">
        </div>
    </header>

    <section id="Article-container"
        class="max-w-[1130px] sm:max-lg:mx-[20px] lg:mx-auto mx-[10px] lg:flex lg:place-items-start place-items-center gap-20 mt-[50px]">
        ...
        <div class="side-bar flex flex-col w-[300px] shrink-0 gap-10 lg:mt-[0px] mt-[40px]">
            <div class="ads flex flex-col gap-3 w-full">
                <a href="#">
                    <img src="{{ asset('portal-berita/src/assets/images/iklans/banner.png') }}"
                        class="object-contain w-full h-full" alt="ads" />
                </a>
            ...
            <div class="ads flex flex-col gap-3 w-full">
                <a href="">
                    <img src="{{ asset('portal-berita/src/assets/images/iklans/banner1.png') }}"
                        class="object-contain w-full h-full" alt="ads" />
                </a>
                <p class="font-medium text-sm leading-[21px] text-[#A3A6AE] flex gap-1">
                    Our Advertisement <a href="#" class="w-[18px] h-[18px]"><img
    ...
    <section id="Advertisement" class="max-w-[1130px] mx-auto flex justify-center mt-[40px]">
        <div class="flex flex-col gap-1 shrink-0 w-fit">
            <a href="#">
                <div
                    class="sm:max-xl:w-[600px] sm:max-xl:h-[70px] xl:w-full xl:h-full h-[50px] w-[350px] flex shrink-0 border border-[#EEF0F7] sm:max-2xl:rounded-2xl rounded-xl overflow-hidden">
    ...
    <section id="Up-to-date" class="w-full flex justify-center mt-[70px] py-[50px] bg-[#F9F9FC] mb-0">
        <div class="max-w-[1130px] lg:mx-auto flex flex-col gap-[30px] sm:max-lg:mx-[30px] mx-[20px]">
            <div class="flex justify-between items-center">
                <h2
                    class="font-bold sm:max-2xl:text-[20px] text-[14px] leading-relaxed badge-orange rounded-full sm:max-lg:p-[6px_16px] lg:p-[8px_18px] p-[5px_14px] bg-[#FFECE1] text-[#FF6B18] w-fit">
                    Other News You Might Be Interested...
                </h2>
            </div>
            <div class="grid sm:max-2xl:grid-cols-3 grid-cols-1 gap-[30px]">
                    ...
                    <a href="{{ url('/post/' . $post->slug) }}" class="card-news">
                        <div
                            ...
                            <div class="card-info flex flex-col gap-[6px]">
                                ...
                                <p class="text-sm leading-[21px] text-[#A3A6AE]">{{ $post->created_at->format('d M, Y') }}
                                </p>
                            </div>
                        </div>
                    </a>
